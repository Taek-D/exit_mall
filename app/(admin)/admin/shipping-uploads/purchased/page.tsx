...
export default async function AdminPurchasedShippingPage({
  searchParams,
}: {
  searchParams: { status?: string };
}) {
  const status = (searchParams.status ?? 'all') as ShippingUploadStatus | 'all';
  const { rows, counts: c } = await fetchAdminShippingUploads(status, 'purchased');
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        <StatusTabs
          ...
          counts={c}
          hrefFor={(key) =>
            `/admin/shipping-uploads/purchased${key === 'all' ? '' : `?status=${key}`}`
          }
        />
              ...
              <tbody>
                {rows.map((u) => (
                  <tr key={u.id} className="border-t h-11 hover:bg-surface-muted/60">
                    <td className="px-4">
                      <Link
                        href={`/admin/shipping-uploads/purchased/${u.id}`}
                    ...
                    <td className="px-3 text-right">
                      <Link href={`/admin/shipping-uploads/purchased/${u.id}`} aria-label="상세">
