  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
        <strong>Legacy 주문</strong> — 옛 일반 주문 흐름. 신규 주문은{' '}
      ...
      <div className="flex items-end justify-between gap-4">
        ...
        <a
          href={`/admin/orders-legacy/export${status !== 'all' ? `?status=${status}` : ''}`}
          ...
          <div className="flex min-w-max">
            {TABS.map((t) => {
              ...
              return (
                <Link
                  key={t.key}
                  href={`/admin/orders-legacy${t.key === 'all' ? '' : `?status=${t.key}`}`}
                  className={cn(
