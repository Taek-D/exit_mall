...
import { callRpc, formatZodError, mutationTable, revalidatePaths, type ActionResult } from '@/lib/actions/_shared';
...
export async function adjustBalanceAction(userId: string, fd: FormData): Promise<ActionResult> {
  ...
  if (error) {
    ...
    return { ok: false, error: '예치금 조정에 실패했습니다.' };
  }
  revalidatePaths([`/admin/users/${userId}`]);
  return { ok: true };
}

export async function updateThresholdAction(userId: string, fd: FormData): Promise<ActionResult> {
  const guard = await requireAdmin();
  if (!guard.ok) return { ok: false, error: guard.error };
  ...
  if (error) {
    ...
    return { ok: false, error: '알림 기준을 저장하지 못했습니다.' };
  }
  revalidatePaths([`/admin/users/${userId}`]);
  return { ok: true };
}

export async function updateUserContactAction(userId: string, fd: FormData): Promise<ActionResult> {
  const guard = await requireAdmin();
  if (!guard.ok) return { ok: false, error: guard.error };
  ...
  if (error) {
    ...
    return { ok: false, error: '사용자 기본정보를 저장하지 못했습니다.' };
  }
...
export async function setUserStatusAction(userId: string, status: 'active'|'suspended'): Promise<ActionResult> {
  const guard = await requireAdmin();
  if (!guard.ok) return { ok: false, error: guard.error };
  if (guard.user.id === userId) return { ok: false, error: '본인 상태는 변경할 수 없습니다' };
  ...
  if (error) {
    ...
    return { ok: false, error: '계정 상태를 변경하지 못했습니다.' };
  }
  revalidatePaths([`/admin/users/${userId}`]);
  return { ok: true };
}

export async function setUserGroupAction(userId: string, group: UserGroup): Promise<ActionResult> {
  const guard = await requireAdmin();
  if (!guard.ok) return { ok: false, error: guard.error };
  if (!isUserGroup(group)) return { ok: false, error: '그룹이 올바르지 않습니다.' };
  if (isSelfUserGroupChange(guard.user.id, userId)) return { ok: false, error: '본인 그룹은 변경할 수 없습니다.' };
  ...
  if (before.status !== 'active' || before.role === 'admin') {
    return { ok: false, error: '그룹을 변경할 수 없는 사용자입니다.' };
  }
  ...
  if (error) {
    ...
    return { ok: false, error: '그룹을 변경하지 못했습니다.' };
  }
