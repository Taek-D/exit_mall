-- approve_shipping_upload P1 hardening (codex review round 2):
-- Restore the product_code↔ID consistency guard that existed in
-- 20260509000002_codex_round2_hardening.sql but was inadvertently dropped
-- when 20260513000015 replaced the function for custom-inventory support.
--
-- Without this guard, a tampered order_uploads row could show one name in
-- the uploaded sheet (product_code) while deducting a different inventory
-- ID at approval time, breaking inventory/accounting integrity.
--
-- Equivalent guard added for custom_inventory_id: each item's product_code
-- must match the name of the user_custom_inventory row it references.
...
begin
  ...
  -- 한 행에 product_id 와 custom_inventory_id 가 둘 다 있으면 명시적 거부
  if exists (
  ...
  -- product_code ↔ (product_id | custom_inventory_id) 일관성 가드:
  -- items 의 ID 가 가리키는 행의 name 이 product_code 와 같아야 한다.
  -- product_id 행은 products.name 으로, custom_inventory_id 행은
  -- user_custom_inventory(user_id=v_upload.user_id).name 으로 검증.
