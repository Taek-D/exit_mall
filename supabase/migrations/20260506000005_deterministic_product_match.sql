-- Codex P2 follow-up: approve_order_upload's product lookup uses `LIMIT 1` without
-- ordering, so when multiple products share the same name (no unique constraint exists),
-- the matched product is non-deterministic — the same upload could resolve to different
-- products on retry, including products not intended for the order.
--
-- Fix: restrict the lookup to active products and pick a stable winner via
-- ORDER BY created_at DESC, id DESC (most recently created active product wins). Apply
-- the same rule to the pre-validation aggregation so that batch-quantity check, item
-- INSERT, and per-user-limit accounting all reference the same row.
...
begin
  ...
  -- Pre-validate per_user_limit by aggregating quantities per matched product.
  -- LATERAL ensures each input row resolves to exactly one product row deterministically.
  for v_check in
    with input_rows as (
    ...
    matched as (
      select ir.quantity, p.id as product_id, p.per_user_limit
      from input_rows ir
      cross join lateral (
  ...
  for v_item in select * from jsonb_array_elements(v_upload.items) loop
    ...
    -- Deterministic match: most recent active product with this name wins.
